...
class retweet(Extension):
    ...
    async def retweet(self):
        ...
        user: str = "imasml_theater"
        cookies: dict = {"auth_token": getenv("auth_token")}
        url: str = f"https://syndication.twitter.com/srv/timeline-profile/screen-name/{user}"
        ...
        while queue:
            tweetId = queue.pop()
            api_callback: dict = await fetch_tweet(tokens, tweetId, host="twitter")
