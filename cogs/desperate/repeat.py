...
class post(Extension):
    ...
    @slash_command(name="post", description="匿名發文（可使用超連結文字格式）")
    @slash_option(name="content", description="想要問的問題", type=OptionType.STRING, required=True)
    async def post(self, ctx: SlashContext, content: str):
        send = interactions.Button(style=interactions.ButtonStyle.PRIMARY, label="Yes, send this message.", custom_id="send")
